...
export class ShopDataWriter extends Writer {
  ...
  async generateData(): Promise<void> {
    ...
    this.data = shopGoodsExcelConfigLoader.data.map(shopGood => <ShopData>Object.fromEntries(Object.entries(<ShopData>{
      Id: shopGood.GoodsId,
      ShopType: shopGood.ShopType,
      ItemCount: shopGood.ItemCount,
      CostItems: <{ Id: number, Count: number }[]>shopGood.CostItems.filter(costItem => costItem.Id != null && costItem.Count != null),
